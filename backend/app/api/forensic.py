...
async def get_forensic_queue(current_user: dict = Depends(require_roles(UserRole.FORENSIC_ANALYST))):
    ...
    for evidence in all_evidence:
        ...
        is_pending = (
            evidence.get("status") == "TRANSFERRED_TO_FORENSIC" or
            evidence.get("analysis_status") == "PENDING" or
            (evidence.get("analysis_status") is None and evidence.get("status") != "COMPLETED")
        )
...
# backend/app/api/forensic.py - UPDATE the analyze endpoint
...
async def create_forensic_report(
    ...
    report_data = {
        ...
        "methodology": getattr(payload, "methodology", ""),  # ← ADD THIS
        "equipment_used": getattr(payload, "equipment_used", ""),  # ← ADD THIS
    ...
    try:
        ipfs_cid = await ipfs_client.upload_json(report_data)
        report_hash = ipfs_client.generate_hash(report_data)
        report_data["ipfs_cid"] = ipfs_cid
        report_data["hash"] = report_hash
    except Exception as e:
        print(f"IPFS upload failed: {e}")
        report_data["ipfs_cid"] = "UPLOAD_FAILED"
        report_data["hash"] = ipfs_client.generate_hash(report_data)
    ...
    anchor_sha256(
        object_type="FORENSIC_REPORT",
        object_id=report_id,
        sha256_hex=report_data.get("hash", ""),
        ipfs_cid=report_data.get("ipfs_cid", ""),
    )
...
async def transfer_report_to_court(
    ...
    if evidence:
        ...
        ipfs_storage.save_evidence(report.get("evidence_id"), evidence)
...
async def get_forensic_reports(current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] in [UserRole.FORENSIC_ANALYST.value, UserRole.COURT.value, UserRole.PUBLIC_AUDITOR.value]:
...
async def get_forensic_report(report_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] == UserRole.INVESTIGATOR.value:
        case = ipfs_storage.get_case(report.get("case_id"))
...
async def get_forensic_cases(current_user: dict = Depends(require_roles(UserRole.FORENSIC_ANALYST))):
    ...
    for case in all_cases:
        ...
        for ev in case_evidence:
            if ev.get("transferred_by"):  # Investigator who sent to forensic
                investigator_email = ev.get("transferred_by")
                investigator_name = ev.get("transferred_by_name")
                break  # Use first investigator found
...
async def get_case_investigator(
    ...
    # Find first evidence submitted to forensic
    for ev in case_evidence:
...
async def get_pending_forensic_cases(current_user: dict = Depends(require_roles(UserRole.FORENSIC_ANALYST))):
    """Get all cases pending forensic acceptance (NOT accepted yet)"""
    all_cases = ipfs_storage.get_all_cases()
    all_evidence = ipfs_storage.get_all_evidence()
    
    pending_cases = []
    for case in all_cases:
        # Skip if case is already accepted
        if case.get("forensic_status") == "ACCEPTED":
            continue
        
        # Check if case has evidence sent to forensic
        case_evidence = [ev for ev in all_evidence if ev.get("case_id") == case.get("case_id")]
        ...
        if has_forensic_pending:
            ...
            if case.get("investigator_email"):
                investigator = ipfs_storage.get_user(case.get("investigator_email"))
...
async def forensic_case_action(
    ...
    if payload.action == "ACCEPT":
        # Update ALL evidence for this case (CASE LEVEL)
        for ev in case_evidence:
            if ev.get("status") == "TRANSFERRED_TO_FORENSIC" or ev.get("analysis_status") in ["PENDING", None]:
                ...
                ev["case_accepted_at"] = action_timestamp  # Mark case-level acceptance
                ipfs_storage.save_evidence(ev.get("evidence_id"), ev)
        
        # Update CASE forensic status
...
async def get_full_case_for_forensic(
    ...
    # Get ALL evidence for this case - FIXED
    all_evidence = ipfs_storage.get_all_evidence()
    case_evidence = []
    ...
    # Prepare evidence with full details
    evidence_with_details = []
    for ev in case_evidence:
        # Find forensic report for this evidence
        report = None
...
async def get_accepted_forensic_cases(current_user: dict = Depends(require_roles(UserRole.FORENSIC_ANALYST))):
    ...
    accepted_cases = []
    for case in all_cases:
        # Check if case has accepted evidence
        case_evidence = [ev for ev in all_evidence if ev.get("case_id") == case.get("case_id")]
...
{'='*60}
Generated by Decentralized Justice System
{'='*60}
"""
    return Response(
...
class ShareReportRequest(BaseModel):
    ...
    share_with_role: str  # INVESTIGATOR, COURT, POLICE
...
async def share_forensic_report(
    ...
    target_user = ipfs_storage.get_user(payload.share_with_email)
    if not target_user:
        raise HTTPException(status_code=404, detail="User not found")
    
    # Initialize shared_with list
    ...
    if case:
        ...
        # Add share event to case timeline
        ...
        # Also update shared_reports list if exists
        ...
        # Save updated case
        ipfs_storage.save_case(report.get("case_id"), case)
