...
async def get_investigator_dashboard(current_user: dict = Depends(require_roles(UserRole.INVESTIGATOR))):
    ...
    for i in range(7):
        ...
        for case in my_cases:
            try:
                case_date = datetime.fromisoformat(case.get("created_at", "")).date()
                if case_date == day:
                    day_cases += 1
            except (ValueError, TypeError):
                pass
...
async def get_performance_metrics(current_user: dict = Depends(require_roles(UserRole.INVESTIGATOR))):
    ...
    for case in my_cases:
        case_tasks = await mongo_storage.get_case_tasks(case.get("case_id"))
    ...
    # Get reports count
    reports = await mongo_storage.get_all_reports()
